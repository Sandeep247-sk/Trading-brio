...
export const metadata: Metadata = {
  title: "Strategy Builder | Trade OS",
};
...
export default async function StrategyPage({
    ...
    return (
      <div className="space-y-6 animate-fade-in max-w-5xl">
        <div className="flex items-center gap-4">
          <Link
            href="/strategy"
            className="p-2 border border-border hover:border-gray-750 bg-card text-muted-foreground hover:text-white rounded-md transition"
          >
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {strategies.length === 0 ? (
        <div className="bg-card border border-border rounded-lg p-16 text-center max-w-3xl mx-auto space-y-4">
          <div className="p-4 bg-muted/60 border border-border rounded-full inline-block">
            <Target className="h-8 w-8 text-gray-650" />
          </div>
          <div>
            <h3 className="text-base font-semibold text-foreground">No strategy defined</h3>
        ...
        <div className="space-y-6">
          {strategies.map((strat) => {
            ...
            return (
              ...
              >
                {/* Strat top banner */}
                <div className="border-b border-border bg-muted/10 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div>
                    <div className="flex items-center space-x-3">
                      <h3 className="text-base font-bold text-white">{strat.name}</h3>
                      <span className="px-2 py-0.5 bg-muted border border-border text-muted-foreground text-[10px] font-bold rounded">
                        {strat.market}
                      </span>
                ...
                <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3 bg-muted/10 border border-border/50 p-4 rounded-md text-xs space-y-3">
                      <div className="space-y-1">
                        <span className="text-muted-foreground">Sessions</span>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {preferredSessions.map((s) => (
                            <span key={s} className="px-2 py-0.5 bg-muted border border-border text-foreground/80 rounded text-[10px] font-semibold">
                              {s.replace(/_/g, " ")}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="space-y-1">
                        <span className="text-muted-foreground">Higher Timeframes</span>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {higherTimeframes.map((s) => (
                            <span key={s} className="px-2 py-0.5 bg-muted border border-border text-foreground/80 rounded text-[10px] font-mono font-bold">
                              {s}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="space-y-1">
                        <span className="text-muted-foreground">Execution Timeframes</span>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                    ...
                    <div className="space-y-3 bg-muted/10 border border-border/50 p-4 rounded-md text-xs space-y-3">
                      ...
                      {riskRules.length > 0 && (
                        <div className="space-y-1.5">
                          <span className="text-muted-foreground font-semibold uppercase text-[9px] tracking-wide">Risk Rules</span>
                          {riskRules.map((r, idx) => (
                      ...
                      {managementRules.length > 0 && (
                        <div className="space-y-1.5 pt-1">
                          <span className="text-muted-foreground font-semibold uppercase text-[9px] tracking-wide">Management Rules</span>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="max-h-[175px] overflow-y-auto space-y-2 bg-muted/10 border border-border/50 p-4 rounded-md text-xs">
                      {rulesList.length === 0 ? (
                        <span className="text-muted-foreground italic">No checklist rules added.</span>
                      ) : (
                        rulesList.map((rule) => (
                          <div key={rule.id} className="flex items-start gap-1.5 bg-card border border-border p-2 rounded">
                            ...
                            <div className="min-w-0">
                              <p className="font-semibold text-foreground/80 truncate">{rule.name}</p>
                              {rule.description && (
