  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex justify-center">
        <div className="relative flex items-center bg-card/80 rounded-2xl p-1 border border-border/50 backdrop-blur-sm">
          {/* Sliding Indicator */}
          <motion.div
          ...
          <button onClick={() => handleTabSwitch("risk")}
            className={`relative z-10 flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold transition-colors duration-200 ${
              activeTab === "risk" ? "text-blue-400" : "text-muted-foreground hover:text-foreground/80"
            }`}>
          ...
          <button onClick={() => handleTabSwitch("lot")}
            className={`relative z-10 flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold transition-colors duration-200 ${
              activeTab === "lot" ? "text-blue-400" : "text-muted-foreground hover:text-foreground/80"
            }`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2 border-b border-border/60 pb-3">
                  ...
                  <h3 className="text-sm font-semibold text-foreground">Position Calculator</h3>
                </div>

                {/* Account Balance */}
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Account Balance ({currency})</label>
                ...
                {/* Instrument Type */}
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Instrument Type</label>
                  <select value={instrumentType} onChange={(e) => setInstrumentType(e.target.value as typeof instrumentType)}
                    className="calc-input w-full h-11 px-3 rounded-xl text-sm text-foreground/80 font-medium appearance-none cursor-pointer">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Risk Per Trade (%)</label>
                    {maxRiskPerTrade && (
                      <span className="text-[9px] text-muted-foreground/60 font-mono">Max: {Number(maxRiskPerTrade)}%</span>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Entry</label>
                    <input type="number" step="any" placeholder="2350.50" value={entryPrice}
                      onChange={(e) => setEntryPrice(e.target.value)}
                      className="calc-input w-full h-11 px-3 rounded-xl text-xs text-foreground font-mono" />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input type="number" step="any" placeholder="2345.00" value={stopLoss}
                      ...
                      className="calc-input w-full h-11 px-3 rounded-xl text-xs text-foreground font-mono" style={{ borderColor: "rgba(239,68,68,0.15)" }} />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <input type="number" step="any" placeholder="2365.00" value={takeProfit}
                      ...
                      className="calc-input w-full h-11 px-3 rounded-xl text-xs text-foreground font-mono" style={{ borderColor: "rgba(34,197,94,0.15)" }} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider flex items-center gap-1">
                      Lot Size (Units) <Info className="h-3 w-3 text-muted-foreground/50" />
                    </label>
                    <select value={lotSize} onChange={(e) => setLotSize(parseInt(e.target.value))}
                      className="calc-input w-full h-11 px-3 rounded-xl text-xs text-foreground/80 appearance-none cursor-pointer">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Pip Value ({symbol}/pip)</label>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="result-card-glow rounded-xl p-4">
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      <p className="text-[9px] text-muted-foreground uppercase font-bold">Risk Amount</p>
                    </div>
                    <p className="text-xl font-bold font-mono text-red-400">
                      {symbol}{calculations.riskAmount.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                    <p className="text-[9px] text-muted-foreground/60 mt-1">{riskPercent}% of {symbol}{balance.toLocaleString()}</p>
                  </div>

                  <div className="result-card-glow rounded-xl p-4">
                    <div className="flex items-center gap-1.5 mb-2">
                      <TrendingUp className="h-3.5 w-3.5 text-green-400" />
                      <p className="text-[9px] text-muted-foreground uppercase font-bold">Expected Reward</p>
                    </div>
                  ...
                  <div className="result-card-glow rounded-xl p-4">
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      <p className="text-[9px] text-muted-foreground uppercase font-bold">Position Size</p>
                    </div>
                  ...
                  <div className="result-card-glow rounded-xl p-4">
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      <p className="text-[9px] text-muted-foreground uppercase font-bold">R:R Ratio</p>
                    </div>
                    <p className={`text-xl font-bold font-mono ${
                      calculations.rrRatio >= 2 ? "text-green-400" : calculations.rrRatio >= 1 ? "text-orange-400" : "text-muted-foreground"
                    }`}>
                ...
                {calculations.valid && (
                  <div className="grid grid-cols-2 gap-3">
                    <div className="result-card-glow rounded-xl p-3 flex justify-between items-center">
                      <span className="text-[10px] text-muted-foreground uppercase font-bold">Pips at Risk</span>
                      <span className="text-sm font-bold font-mono text-red-400">{calculations.pipsAtRisk.toFixed(4)}</span>
                    </div>
                    <div className="result-card-glow rounded-xl p-3 flex justify-between items-center">
                      <span className="text-[10px] text-muted-foreground uppercase font-bold">Pips to Target</span>
                ...
                {calculations.rrRatio > 0 && (
                  <div className="result-card-glow rounded-xl p-4 space-y-2">
                    <p className="text-[10px] text-muted-foreground uppercase font-bold">Risk/Reward Visual</p>
