...
export const AccountSelector: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="h-9 w-32 bg-gray-950 border border-gray-850 rounded-md animate-pulse" />
    );
  }
  ...
  if (accounts.length === 0) {
    return (
      <Button
        ...
        className="h-9 gap-1.5 border-dashed border-gray-700 text-gray-400 hover:text-white"
      >
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger render={
        <Button
          variant="outline"
          className="h-9 px-3 bg-gray-950 border-gray-850 hover:bg-gray-900 text-gray-200 gap-2 font-medium text-xs sm:text-sm"
        >
          <Wallet className="h-4 w-4 text-blue-500" />
          <div className="text-left hidden xs:block">
            <p className="leading-none text-[11px] font-semibold text-gray-300">
              {selectedAccount?.name}
            </p>
            <p className="leading-none text-[9px] text-gray-500 mt-0.5 font-mono">
      ...
      <DropdownMenuContent align="end" className="w-56 bg-gray-950 border border-gray-850">
        ...
        {accounts.map((acc) => {
          ...
          return (
            <DropdownMenuItem
              ...
              className="flex items-center justify-between px-2.5 py-2 cursor-pointer hover:bg-gray-900 rounded-md text-xs"
            >
        ...
        <div className="border-t border-gray-850 my-1" />
        <DropdownMenuItem
          onClick={() => router.push("/settings/accounts")}
          className="flex items-center gap-2 px-2.5 py-2 cursor-pointer hover:bg-gray-900 rounded-md text-xs text-blue-400 font-medium"
        >
          <Plus className="h-3.5 w-3.5" />
