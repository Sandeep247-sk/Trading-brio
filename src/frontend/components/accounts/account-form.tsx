...
export const AccountForm: React.FC<AccountFormProps> = ({
  ...
  const [maxWeeklyDrawdown, setMaxWeeklyDrawdown] = useState<string>(
    initialData?.maxWeeklyDrawdown ? String(initialData.maxWeeklyDrawdown) : ""
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        maxDailyDrawdown: maxDailyDrawdown ? parseFloat(maxDailyDrawdown) : null,
        maxWeeklyDrawdown: maxWeeklyDrawdown ? parseFloat(maxWeeklyDrawdown) : null,
        maxOverallDrawdown: maxOverallDrawdown ? parseFloat(maxOverallDrawdown) : null,
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                >
                  ...
                  <option value={AccountType.PROP_CHALLENGE}>Prop Challenge</option>
                  <option value={AccountType.PROP_FUNDED}>Prop Funded</option>
                </select>
              </div>
        ...
        {/* Risk Management / Drawdown Parameters */}
        <div className="space-y-4 bg-gray-900/40 p-5 border border-gray-800 rounded-lg">
          <h3 className="text-sm font-semibold text-gray-250 uppercase tracking-wider">
            Risk & Drawdown Rules
          </h3>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="space-y-1 col-span-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Max Daily Drawdown (%)</label>
                <input
                  type="number"
                  step="0.01"
                  placeholder="e.g. 5.0"
              ...
              <div className="space-y-1 col-span-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Max Weekly Drawdown (%)</label>
                <input
                  type="number"
                  step="0.01"
                  placeholder="e.g. 8.0"
                  value={maxWeeklyDrawdown}
                  onChange={(e) => setMaxWeeklyDrawdown(e.target.value)}
                  className="w-full h-10 px-2 bg-gray-950 border border-gray-850 rounded text-xs text-gray-300 focus:outline-none focus:border-blue-500 font-mono"
                />
              </div>

              <div className="space-y-1 col-span-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase">Max Overall Drawdown (%)</label>
                <input
                  type="number"
                  step="0.01"
            ...
            <div className="pt-2 text-xs text-gray-500 leading-relaxed">
              * Configure drawdowns to reflect prop-firm constraints or personal risk ceilings. The application alerts you when you are nearing these drawdown thresholds.
            </div>
          </div>
        </div>
      </div>
